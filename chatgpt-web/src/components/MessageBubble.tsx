...
export const MessageBubble = memo<MessageBubbleProps>(({ msg, isTyping }) => {
  ...
  useEffect(() => {
    if (isTypingStyleBubble) {
      ...
      const timeoutId = setTimeout(() => {
        setDisplayedContent(
          msg.content.substring(0, displayedContent.length + 1),
        );
      }, typingSpeed);
  ...
  const isCurrentlyTyping =
    isTypingStyleBubble && displayedContent.length !== msg.content.length;
  const contentToRender = isCurrentlyTyping
    ? displayedContent + "▍"
    : msg.content;
  ...
  return (
    ...
    >
      <div className="flex flex-col max-w-2xl min-w-0">
        ...
        <span
          className={`text-xs text-gray-500 dark:text-gray-400 mt-1 px-2 ${msg.role === "user" ? "text-right" : "text-left"}`}
        >
